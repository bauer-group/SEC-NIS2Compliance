...
| 步骤 | 描述 | 责任人 |
|---------|-------------|---------------|
| 1. 需求概况 | 基于保护需求定义安全要求 | 业务部门 + ISB |
| 2. 信息收集 | 获取安全文档、认证、参考资料 | 采购 |
| 3. 评估 | 按评估标准进行审查 | ISB |
| 4. 风险评估 | 确定残余风险、制定措施 | ISB |
| 5. 决策 | 批准或拒绝 | ISB + 管理层（针对关键供应商） |
...
| 合同设计 | 15% | 0-3（标准 / 已调整 / 含审计权 / 完整） |
| 分包商透明度 | 10% | 0-3（无信息 / 清单 / 需审批 / 合同约定） |
| 历史记录 | 15% | 0-3（未知 / < 1年 / 1-3年 / > 3年） |

**总分：** 各标准得分 × 权重，求和——得出0至3之间的加权平均值。

**最低分值：** 标准供应商2.0，关键供应商2.5（若所有标准权重相同，则分别相当于12/18分和15/18分）。

::: info 评分方法
每项标准按0至3分评分，并乘以其权重；各项之和即为加权总分（0至3）。因此，缺少认证（25%）的影响大于分包商透明度不足（10%）。低于最低分值的供应商必须提升其安全水平，否则将被拒绝。例外情况须经ISB和管理层批准，并记录风险接受情况。
:::
